...
const negativeTests = [
    { testName: 'Price: Low to High', constSelector:() => InventoryPage.itemPrice, sortingMethod: () => InventoryPage.sortByLoHiClick(), SortingFunction: (a: string, b: string) => parseFloat(a.replace('$', '').replace(',', '')) - parseFloat(b.replace('$', '').replace(',', '')) },
    { testName: 'Price: High to Low', constSelector:() => InventoryPage.itemPrice, sortingMethod: () => InventoryPage.sortByHiLoClick(), SortingFunction: (a: string, b: string) => parseFloat(b.replace('$', '').replace(',', '')) - parseFloat(a.replace('$', '').replace(',', '')) },
    { testName: 'Name: A to Z', constSelector:() => InventoryPage.itemName, sortingMethod: () => InventoryPage.sortByNameAZ(), SortingFunction: (a: string, b: string) => a.localeCompare(b) },
    { testName: 'Name: Z to A', constSelector:() => InventoryPage.itemName, sortingMethod: () => InventoryPage.sortByNameZA(), SortingFunction: (a: string, b: string) => b.localeCompare(a) },
];
negativeTests.forEach(({ testName, constSelector, sortingMethod, SortingFunction }) => {
